...
{
  // echoes stuff to the outside terminal
...
char *p = "\n\rThis awesome serial port is so ready\n\r\007";
...
int do_rx(struct stty *tty)   /* interrupts already disabled */
{
  ...
  // Write code to put c into inbuf[ ];
  sputc(tty, c);
  bputc(tty);
...
int sgetc(struct stty *tty)
{
  ...
  c = tty->inbuf[tty->inhead];
  tty->inhead = (tty->inhead + 1) % BUFLEN;
  return(c);
}
...
{
  // goes off when you have to put something on an outside terminal
...
{
  // puts stuff in the output
  V(tty);

  // write code to put c into tty's outbuf[ ]
  tty->outtail = (tty->outtail + 1) % BUFLEN;
  tty->outbuf[tty->outtail] = c;
...
int usgets(char *y)
{
  ...
  // Going with the other way for now
  // sgetline(tty, buf);
  // c = buf;
  n = 0;      
  while ( (c = sgetc()) != '\n'){
...
int ksin(char *y)
{
  ...
  // // We now have to send it to umode
  index = 0;
  c = buf[index];
  while(c)
  {
    put_byte(c, running->uss, (y + index));
    index++;
    c = buf[index];
  }
...
int ksout(char *y)
{
  ...
  // printf("About to write %s to serial port 0.\n", buf);
  // Now we write the line to serial port 0
  sputline(&stty[0], buf);
}